...
ENV VITE_EARTH_FRONTEND_HOST=${VITE_LINK_TO_EARTH_HOST}
ENV VITE_MARS_FRONTEND_HOST=${VITE_LINK_TO_MARS_HOST}
ENV VITE_INIT_FRONTEND_HOST=${VITE_LINK_TO_INIT_HOST}
RUN --mount=type=bind,source=package.json,target=package.json \
    --mount=type=bind,source=package-lock.json,target=package-lock.json \
    --mount=type=cache,target=/root/.npm \
    npm ci
